...
namespace graphlab {
  ...
  /** \brief A directed graph data structure which is distributed across
   * multiple machines.
   *
   * This class implements a distributed directed graph data structure where
   * vertices and edges may contain arbitrary user-defined data types as
...
//#endif
    ...
    struct local_edge_list_disk_type;
    ...
    public:
      ...
      lvid_type source_lvid() const {
    	  return edge.source_lvid();
      }

      lvid_type target_lvid() const {
    	  return edge.target_lvid();
      }
    ...
    /**
     * \brief disk. The size of vertices' buffer.
     */
    size_t ver_buf_block_num;
    size_t ver_block_size;

    /**
     * \bried disk. The size of each edge block, i.e. the number of source/target vertices.
     */
 ...
 ****************************************************************************/
    ...
    //disk, only used for disk version to iteration computation in sync engine.
    local_edge_list_disk_type l_in_edges_disk(const lvid_type lvid) {
    	return local_edge_list_disk_type(*this, local_graph.in_edges_disk(lvid));
    }

    //disk, only used for disk version to iteration computation in sync engine.
    local_edge_list_disk_type l_out_edges_disk(const lvid_type lvid) {
    	return local_edge_list_disk_type(*this, local_graph.out_edges_disk(lvid));
    }
...
#pragma omp parallel for
    ...
    struct local_vertex_type {
      ...
      local_edge_list_disk_type in_edges_disk() {
    	  return graph_ref.l_in_edges_disk(lvid);
      }

      local_edge_list_disk_type out_edges_disk() {
    	  return graph_ref.l_out_edges_disk(lvid);
      }
    ...
    struct local_edge_list_type {
      ...
      local_edge_list_type(distributed_graph& graph_ref,
    	  ...
    	  //logstream(LOG_INFO) << "old 1. request in_edges" << std::endl;
    	  //logstream(LOG_INFO) << "old size=" << elist.size() << std::endl;
      }
    ...
    /** \internal
         * \brief disk. A list of edges. Used by l_in_edges() and l_out_edges()
         */
        struct local_edge_list_disk_type {
          make_local_edge_type_functor me_functor;
          typename local_graph_type::edge_list_disk_type elist;

          typedef boost::transform_iterator<make_local_edge_type_functor,
							  typename local_graph_type::edge_list_disk_type::iterator> iterator;
          typedef iterator const_iterator;

          local_edge_list_disk_type(distributed_graph& graph_ref,
                               typename local_graph_type::edge_list_disk_type elist) :
                              me_functor(graph_ref), elist(elist) {
          }
          /// \brief Returns the number of edges in the list
  ...
  public:
    ...
    //disk
    void initLocalGraph () {
    	local_graph.initLocalDiskArgs(useVerDisk, useEdgeDisk, (int)ver_block_size,
    			(int)ver_buf_block_num, (int)edge_block_size);
    }

    //disk
    void resetBefIte() {
    	local_graph.resetLocalBefIte();
    }

    //disk
    long getIOBytes_Read() {
    	return local_graph.getLocalIOBytes_Read();
    }

    //disk
    long getIOBytes_Write() {
    	return local_graph.getLocalIOBytes_Write();
    }

    /** Open R/W model before execute_applys(), enable updating vertices (master and mirror)
     *  disk.
     *  */
    void openReadWriteModel() {
    	local_graph.openLocalReadWriteModel();
    }

    /**
     * Close R/W model after execute_applys(), to skip the write operations to improve the I/O-efficiency.
     * Once close R/W model, all vertices (master and mirror) are read-only.
     * disk
     */
    void closeReadWriteModel() {
    	local_graph.closeLocalReadWriteModel();
    }

    //disk
    void clearTmpFiles() {
    	local_graph.clearLocalTmpFiles();
    }

    //disk
    void save_vertices(bool useVerDisk) {
    	local_graph.save_vertices(useVerDisk);
    }

    //disk
    void save_edges(bool useEdgeDisk) {
    	local_graph.save_edges(useEdgeDisk);
    }
