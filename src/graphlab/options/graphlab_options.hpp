...
namespace graphlab {
  ...
  public:
    ...
    //! Add by wzg, vertex_buffer_size
    size_t ver_buf_block_num;

    //! Add by wzg, edge block size, including how many source/destination vertices per block
    size_t edge_block_size;

    //! sigmod disk, store vertices on disk and use the vertex buffer? 0--false, 1--true
    size_t useVerDisk;

    //! sigmod disk, store edges on disk? 0--false, 1--true
    size_t useEdgeDisk;
    ...
    void setVerBlockSize(size_t size) {
    	ver_block_size = size;
    }

    size_t getVerBlockSize() const {
    	return ver_block_size;
    }

    //! Add by wzg, set the size of vertex buffer.
    void setVerBufBlockNum(size_t _size) {
    	ver_buf_block_num = _size;
    }

    //! Add by wzg, get the size of vertex buffer.
    size_t getVerBufBlockNum() const {
    	return ver_buf_block_num;
    }

    //! Add by wzg, set the size of each edge-block.
    void setEdgeBlockSize(size_t _size) {
    	edge_block_size = _size;
	}

    //! Add by wzg, get the size of each edge block.
    size_t getEdgeBlockSize() const {
    	return edge_block_size;
    }

    //! disk, set the useVerDisk (0--false, 1--true)
    void setUseVerDisk(size_t _useVerDisk) {
    	useVerDisk = _useVerDisk;
    }

    //! disk, get the useVerDisk (0--false, 1--true)
    size_t getUseVerDisk() const {
    	return useVerDisk;
    }

    //! disk, set the useEdgeDisk (0--false, 1--true)
    void setUseEdgeDisk(size_t _useEdgeDisk) {
    	useEdgeDisk = _useEdgeDisk;
    }

    //! disk, get the useEdgeDisk (0--false, 1--true)
    size_t getUseEdgeDisk() const {
    	return useEdgeDisk;
    }
