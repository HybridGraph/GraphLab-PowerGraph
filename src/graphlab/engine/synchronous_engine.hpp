...
namespace graphlab {
  ...
  private:
    ...
    /**
     * \brief The size of each ver block.
     */
    size_t ver_block_size;

    /**
     * \brief The size of vertices' buffer.
     */
    size_t ver_buf_block_num;

    /**
     * \bried The size of each edge block, i.e. the number of source/target vertices.
     */
    ...
    */
    ...
    /**
     * \brief Bit indicating whether a vertex (master&mirror) indeed change.
     * True, means should be involved in gather, otherwise, not be involved in gather.
     */
    ...
    /**
     * \brief The i/o bytes of reading vertices&edges per iteration.
     */
    atomic<long> io_bytes_read;

    atomic<long> io_bytes_read_job;

    /**
     * \brief The i/o bytes of writing vertices per iteration.
     */
    atomic<long> io_bytes_write;

    atomic<long> io_bytes_write_job;
  ...
  synchronous_engine(distributed_control &dc,
    ...
    ncpus((size_t)1),
    ver_block_size(opts.getVerBlockSize()),
    ver_buf_block_num(opts.getVerBufBlockNum()),
    edge_block_size(opts.getEdgeBlockSize()),
    threads(2*1024*1024 /* 2MB stack per fiber*/),
    //thread_barrier(opts.get_ncpus()),
    thread_barrier((size_t)1),
    ...
    per_thread_compute_time.resize((size_t)1);
    use_cache = false;
    ...
    useVerDisk = (opts.getUseVerDisk()==(size_t)1)? true:false;
    useEdgeDisk = (opts.getUseEdgeDisk()==(size_t)1)? true:false;

    if (rmi.procid() == 0) {
    	if (useVerDisk) {
    		logstream(LOG_EMPH) << "Disk Option: useVerDisk = true" << std::endl;
    		logstream(LOG_EMPH) << "Disk Option: vertex_block_size = " << ver_block_size << std::endl;
    		logstream(LOG_EMPH) << "Disk Option: vertex_buffer_block_num = " << ver_buf_block_num << std::endl;
    	} else {
    		logstream(LOG_EMPH) << "Disk Option: useVerDisk=false" << std::endl;
    	}
    	if (useEdgeDisk) {
    		logstream(LOG_EMPH) << "Disk Option: useEdgeDisk = true" << std::endl;
    		logstream(LOG_EMPH) << "Disk Option: edge_block_size = " << edge_block_size << std::endl;
    	} else {
    		logstream(LOG_EMPH) << "Disk Option: useEdgeDisk=false" << std::endl;
    	}
    }
  ...
  synchronous_engine<VertexProgram>::start() {
	  graph.save_vertices(useVerDisk);
	  graph.save_edges(useEdgeDisk);
	  rmi.barrier();
    ...
    //disk
    num_of_vertices = (int)graph.num_local_vertices();
    is_changed = new bool[num_of_vertices];
    if (sig_all) {
    ...
    float last_print = -5;
    if (rmi.procid() == 0) {
      logstream(LOG_EMPH) << "Iteration counter will output per iteration."
                        << std::endl;
    }
    io_bytes_read_job = 0;
    io_bytes_write_job = 0;

    graphlab::timer compTimer;
    compTimer.start();
    float compStartTime = compTimer.approx_time_seconds();
    // Program Main loop ====================================================
    while(iteration_counter < max_iterations && !force_abort ) {
    	graph.resetBefIte();
    	io_bytes_read = 0;
    	io_bytes_write = 0;
    	//logstream(LOG_INFO) << "finish resetBefIte" << std::endl;
      ...
      //logstream(LOG_INFO) << "exchange_messages successfully" << std::endl;
      /**
       * Post conditions:
      ...
      //logstream(LOG_INFO) << "gather successfully" << std::endl;
      ...
      //logstream(LOG_INFO) << "applys successfully" << std::endl;
      /**
       * Post conditions:
      ...
      //logstream(LOG_INFO) << "scatters successfully" << std::endl;
      /**
       * Post conditions:
       *   1) NONE
       */
      ...
      io_bytes_read = graph.getIOBytes_Read();
      io_bytes_write = graph.getIOBytes_Write();
      long total_io_bytes_read = io_bytes_read;
      long total_io_bytes_write = io_bytes_write;
      rmi.all_reduce(total_io_bytes_read);
      rmi.all_reduce(total_io_bytes_write);
      if (rmi.procid() == 0 && print_this_round)
    	  logstream(LOG_EMPH)
    	  << ", Read bytes: " << total_io_bytes_read
    	  << ", Write bytes: " << total_io_bytes_write << std::endl;
      io_bytes_read_job = io_bytes_read_job + total_io_bytes_read;
      io_bytes_write_job = io_bytes_write_job + total_io_bytes_write;
    }// end the iterations

    float compEndTime = compTimer.approx_time_seconds();

    if (rmi.procid() == 0) {
      logstream(LOG_EMPH) << iteration_counter
                        << " iterations completed." << std::endl;
      logstream(LOG_EMPH) << "total read bytes: " << io_bytes_read_job
    		  << ", total write bytes: " << io_bytes_write_job << std::endl;
      logstream(LOG_EMPH) << "total computation time: "
    		  << (compEndTime-compStartTime) << " seconds" << std::endl;
    }
  ...
  execute_gathers(const size_t thread_id) {
    ...
    //logstream(LOG_INFO) << "execute_gathers by thread_id=" << thread_id << "\n";
    while (1) {
      // increment by a word at a time
      lvid_type lvid_block_start =
                  shared_lvid_counter.inc_ret_last(8 * sizeof(size_t));
      ...
      foreach(size_t lvid_block_offset, local_bitset) {
        ...
        if( caching_enabled && has_cache.get(lvid) ) {
          ...
          if(gather_dir == IN_EDGES || gather_dir == ALL_EDGES) {
        	  /*if (rmi.procid() == 0 && context.iteration() == 1) {
        		  logstream(LOG_INFO) << "disk debug. gather in edges, vid=" << vertex.id() << std::endl;
        	  }*/
            if (useEdgeDisk) {
            	foreach(local_edge_type local_edge, local_vertex.in_edges_disk()) {
            		edge_type edge(local_edge);
            		if (!is_changed[((int)edge.source_lvid())]) {
            			continue;
            		}
            		/*if (rmi.procid() == 0 && context.iteration() == 1) {
            			logstream(LOG_INFO) << "disk debug. svid=" << edge.source().id() << ",";
            		}*/
          ...
          if(gather_dir == OUT_EDGES || gather_dir == ALL_EDGES) {
        	//logstream(LOG_INFO) << "disk debug. gather out edges, vid=" << vertex.id() << std::endl;
            if (useEdgeDisk) {
            	foreach(local_edge_type local_edge, local_vertex.out_edges_disk()) {
            		edge_type edge(local_edge);
            		if (!is_changed[((int)edge.target_lvid())]) {
            			continue;
            		}
            		// elocks[local_edge.id()].lock();
            		if(accum_is_set) { // \todo hint likely
    ...
    //logstream(LOG_INFO) << "end_gathers by thread_id=" << thread_id << "\n";
  ...
  execute_applys(const size_t thread_id) {
    ...
    //logstream(LOG_INFO) << "execute_applys by thread_id=" << thread_id << "\n";
    fixed_dense_bitset<8 * sizeof(size_t)> local_bitset;  // allocate a word size = 64bits
    graph.openReadWriteModel();
    while (1) {
      // increment by a word at a time
      lvid_type lvid_block_start =
                  shared_lvid_counter.inc_ret_last(8 * sizeof(size_t));
      ...
      foreach(size_t lvid_block_offset, local_bitset) {
        ...
        /*if (vertex.id() == (vertex_id_type)465581) {
        	logstream(LOG_INFO) << "disk debug. apply for vid=" << vertex.id() << ", val=" << vertex.data() << "\n";
        }*/
        ...
        //disk. changed, only synchronize mirrors if the value is indeed changed.
        if (context.getUpdateFlag()) {
        	is_changed[(int)lvid] = true;
    ...
    //logstream(LOG_INFO) << "end_applys by thread_id=" << thread_id << "\n";
  ...
  execute_scatters(const size_t thread_id) {
    ...
    //logstream(LOG_INFO) << "execute_scatters by thread_id=" << thread_id << "\n";
    while (1) {
      // increment by a word at a time
      lvid_type lvid_block_start =
                  shared_lvid_counter.inc_ret_last(8 * sizeof(size_t));
      ...
      foreach(size_t lvid_block_offset, local_bitset) {
        ...
        if(scatter_dir == OUT_EDGES || scatter_dir == ALL_EDGES) {
          if (useEdgeDisk) {
        	  foreach(local_edge_type local_edge, local_vertex.out_edges_disk()) {
        		  edge_type edge(local_edge);
        		  //logstream(LOG_INFO) << " " << edge.target().id();
        		  // elocks[local_edge.id()].lock();
        		  vprog.scatter(context, vertex, edge);
        		  // elocks[local_edge.id()].unlock();
        	  }
    ...
    //logstream(LOG_INFO) << "end_scatters by thread_id=" << thread_id << "\n";
